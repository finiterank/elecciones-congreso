...
layoutCoordinates <- gplot(tabla.follows)
...
zp1 <- zp1 + geom_text(data = data.frame(layoutCoordinates),  # Add nodes
                        aes(x = x, y = y+1, label=paste("@", names(indices), sep="")), size=2, color="red")  # Customize gradient v
...
#write.csv(tabla.follows, "tabla.follows.csv")
```
...
names(candidatos.df) <- c("twitter", "x", "y", "seguidores",  "genero", "partido", "departamento")
qplot(x,y, data=candidatos.df, colour=partido, size=log(seguidores))
```
